...
class Handler(threading.Thread):
    ...
    def __init__(self, connection, database, data_dir, publish):
        ...
        :param database: the database's drive url
    ...
    def run(self):
        ...
        while True:
            ...
            time_epoch = snap.datetime / 1000
            time_string = datetime.datetime.fromtimestamp(time_epoch).strftime("%Y-%m-%d_%H-%M-%S-%f")  # noqa:E501
    ...
    def snapshot_to_json(self, snapshot, context, user_id):
        ...
        In order to not make the json file too big,
        we save the image+depth data into a temp file,
        ...
        snap_dict['color_image']['data'] = str(context.path(color_image_filename))  # noqa:E501
        snap_dict['depth_image']['data'] = str(context.path(depth_image_filename))  # noqa:E501
        # convert datetime to seconds from miliseconds
        snap_dict['datetime'] = int(snap_dict['datetime']) / 1000
        return json.dumps(snap_dict)


def run_server(host, port, publish, database='mongodb://127.0.0.1:27017', data="snaps"):  # noqa:E501
    """
    Run the server at the given host+port, listening to clients,
    reading their snapshots and sending them to consumers.
    ...
    :type publish: function/str
    :param data: the data directory to write the snapshots in,
    defaults to ./snaps.
